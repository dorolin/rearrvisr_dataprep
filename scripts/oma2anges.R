...
## Example (with input option 1):
## Rscript oma2anges.R path/to/genomefiles path/to/OMA/Output
## ------------------------------------------------------------------------


## INPUT OPTION 1: COMMAND-LINE ARGUMENTS
## ======================================

args<-commandArgs(trailingOnly = TRUE)

## directory with genome maps
genomedir<-args[1]
## orthologous groups from OMA
omadir<-args[2]
ogfile<-paste(omadir,"OrthologousGroups.txt",sep="/")

genomefiles<-list.files(path=genomedir,pattern="_genome.txt",full.names=FALSE)
subpfx<-gsub(pattern="_genome.txt",replacement="",x=genomefiles)
nspec<-length(subpfx)

outgenomedir<-genomedir


## INPUT OPTION 2: HARDCODED IN SCRIPT (more control)
## ==================================================

## ## MODIFY THIS PART TO SPECIFY CORRECT DIRECTORY AND SPECIES NAMES >>>>
## ## ------------------------------------------------------------------------
...
## ## directory with input genome maps
## genomedir<-"/Users/doro/Dropbox/Drosophila/dataprep/tmp/input"
## fileext<-rep("_genome.txt",nspec)
## genomefiles<-paste0(subpfx,fileext)

## ## orthologous groups from OMA
## ogfile<-"/Users/doro/Dropbox/Drosophila/test/oma/Output/OrthologousGroups.txt"

## ## output directory for modified genome maps
## outgenomedir<-"/Users/doro/Dropbox/Drosophila/dataprep/tmp/input"

## ## <<<< END MODIFY THIS PART TO SPECIFY CORRECT DIRECTORY AND SPECIES NAMES
## ## ------------------------------------------------------------------------
...
outmarkersfile<-paste(outgenomedir,"Markers",sep="/")
...
for(i in 1:nspec){
    genelists[[i]]<-read.table(paste0(genomedir,"/",genomefiles[i]),
                               as.is=TRUE)
